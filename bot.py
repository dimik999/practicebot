...
def start(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    item1 = types.KeyboardButton('Я парень🙍‍♂️')
    item2 = types.KeyboardButton('Я девушка🙍‍♀️')
    markup.add(item1, item2)
...
def stop(message):
    ...
    if chat_info:
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        item1 = types.KeyboardButton('Следующий диалог➡️')
...
def bot_message(message):
    if message.chat.type == 'private':
        if message.text == '🔍Поиск собеседника' or message.text == 'Следующий диалог➡️':
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            item1 = types.KeyboardButton('Парень🙍‍♂️‍')
            item2 = types.KeyboardButton('Девушка🙍‍♀️')
            item3 = types.KeyboardButton('Рандом👫')
            markup.add(item1, item2, item3)

            bot.send_message(message.chat.id, 'Кого искать?🕵️‍♂️', reply_markup=markup)

        elif message.text == '❌Остановить поиск':
            db.delete_queue(message.chat.id)
            bot.send_message(message.chat.id, 'Поиск остановлен, напишите /menu', reply_markup=main_menu())

        elif message.text == 'Парень🙍‍♂️‍':
